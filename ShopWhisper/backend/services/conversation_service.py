...
class ConversationService:
    ...
    async def to_response_dict(self, conversation: Conversation) -> dict:
        """Build a frontend-compatible conversation response."""
        latest_stmt = (
            select(Message)
            .where(
                and_(
        ...
        user = conversation.__dict__.get("user")
        if user is None:
            user = await self.db.get(User, conversation.user_id)
        user_external_id = user.user_external_id if user else str(conversation.user_id)
        return {
            "id": conversation.id,
            ...
            "platform_type": conversation.platform_type or conversation.channel or "web",
            "status": conversation.status,
            "start_time": conversation.start_time,
            "end_time": conversation.end_time,
            "started_at": conversation.start_time,
            "ended_at": conversation.end_time,
            "last_message_at": latest.created_at if latest else conversation.start_time,
            "last_message_preview": latest.content[:80] if latest else None,
            ...
            "intent_distribution": conversation.intent_distribution,
