...
async def publish_demo_listing(
    ...
    product, estimate, inventory_change = await service.publish_demo_listing(
        ...
        promo_prompt=request.promo_prompt,
        target_platform=request.target_platform,
        color=request.color,
        size=request.size,
    )
...
@router.get("/local-photos", response_model=ApiResponse[list[dict]])
async def list_local_photos():
    """List product material images mounted from D:/python_project/ShopWhisper/photo."""
    if not PHOTO_DIR.exists():
        return ApiResponse(data=[])
    photos = []
    for path in sorted(PHOTO_DIR.iterdir()):
        if path.is_file() and path.suffix.lower() in PHOTO_EXTENSIONS:
            photos.append({"name": path.name, "url": f"/photo/{quote(path.name)}", "size": path.stat().st_size})
    return ApiResponse(data=photos)


@router.post("/local-photos/upload", response_model=ApiResponse[dict])
async def upload_local_photo(file: UploadFile = File(...)):
    """Upload a product material image into the mounted photo folder."""
    ...
    if target.exists():
        target = PHOTO_DIR / f"{target.stem}_{int(target.stat().st_mtime)}{suffix}"
    ...
    return ApiResponse(data={"name": target.name, "url": f"/photo/{quote(target.name)}", "size": target.stat().st_size})
...
async def delete_product(
    ...
    """Soft delete a product so it disappears from active listings."""
    service = ProductSyncService(db, tenant_id)
    product = await service.get_product(product_id)
    if not product:
        return ApiResponse(success=False, error={"code": "NOT_FOUND", "message": "商品不存在"})
    ...
    await db.commit()
    return ApiResponse(data={"message": "商品已下架/删除"})
